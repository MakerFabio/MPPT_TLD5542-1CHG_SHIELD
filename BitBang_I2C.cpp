...
//
// modified adding: I2CReadRegister16 to read 16 bit address register
#ifdef _LINUX_
#include <stdint.h>
#include <stdlib.h>
#include <unistd.h>
...
// otherwise return 1 for success
//
static inline uint8_t i2cByteInCyp(BBI2C *pI2C, uint8_t bLast)
{
uint8_t i;
uint8_t b = 0;
...
static inline void i2cReadCyp(BBI2C *pI2C, uint8_t *pData, int iLen)
{
   while (iLen--)
   {
      *pData++ = i2cByteInCyp(pI2C, iLen == 0);
   } // for each byte
} /* i2cRead() */
...
// otherwise return 1 for success
//
static inline int i2cByteOutCyp(BBI2C *pI2C, uint8_t b)
{
uint8_t i, ack;
uint8_t iSDA = pI2C->iSDA;
uint8_t iSCL = pI2C->iSCL; // in case of bad C compiler
...
static inline int i2cBeginCyp(BBI2C *pI2C, uint8_t addr, uint8_t bRead)
{
   int rc;
   SDA_LOW(pI2C->iSDA); // data line low first
   sleep_us(pI2C->iDelay);
   ...
   rc = i2cByteOutCyp(pI2C, addr); // send the slave address and R/W bit

   return rc;
} /* i2cBegin() */


static inline int i2cWriteCyp(BBI2C *pI2C, uint8_t *pData, int iLen)
{
uint8_t b;
int rc, iOldLen = iLen;
...
// Read N bytes starting at a specific I2C internal register
//
int I2CReadRegister16(BBI2C *pI2C, uint8_t iAddr, uint8_t *uRegister, uint8_t *pData, int iLen)
{
  int rc;
 
  rc = i2cBeginCyp(pI2C, iAddr, 0); // start a write operation: send slave address+write bit (0) 

  if (rc == 1) // slave sent ACK for its address
  {
     rc = i2cWriteCyp(pI2C, uRegister, 2); // write the register we want to read from
     if (rc == 2) // if both the bytes of the reg adress has been sent
     {
       i2cEnd(pI2C);
       rc = i2cBeginCyp(pI2C, iAddr, 1); // start a read operation
       if (rc == 1)
       {
         i2cReadCyp(pI2C, pData, iLen);
       }
     }
...
// Read N bytes
//
int I2CReadCyp(BBI2C *pI2C, uint8_t iAddr, uint8_t *pData, int iLen)
{
  int rc;
